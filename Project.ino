...
void checkHallEffect()
{
  ...
  for(i=0;i<3;i++)
  {
    for(j=0;j<3;j++)
    {
      ...
      {
        board[i][j].piece->prevplayer = board[i][j].piece->player; 
        board[i][j].piece->player = RED; //white piece
      }
      if(analogVal >3)
      {
        board[i][j].piece->prevplayer = board[i][j].piece->player;
        board[i][j].piece->player = BLUE; //black piece
      }
      else
      {
        board[i][j].piece->prevplayer = board[i][j].piece->player;
        board[i][j].piece->player = EMPTY; //no piece
      }
    }
  }
}
...
case "Bishop":
  {
    if (board[iCord][jCord].piece->player == BLUE)
    {
      for (i = 1; i < 2; i++)
      {
        if (board[iCord + i][jCord + i].piece->player == RED) //PIECE on the square is enemy , add coord and break
        {
        ...
        else if (board[iCord + i][jCord + i].piece->player == BLUE) // PIECE moving to is your own piece, you can't move past it.
        {
          digitalWrite(bluepins[iCord + i][jCord + i], HIGH);
          break;
        }
    ...
    else if (board[iCord][jCord].piece->player == RED)
    {
      for (i = 1; i < 2; i++)
      {
        if (board[iCord + i][jCord + i].piece->player == BLUE) //PIECE on the square is enemy , add coord and break
        {
        ...
        else if (board[iCord + i][jCord + i].piece->player == RED) // PIECE moving to is your own piece, you can't move past it.
        {
          digitalWrite(redpins[iCord + i][jCord + i], HIGH);
          break;
        }
